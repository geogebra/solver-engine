...
class ParserTest {
    ...
    fun testLogarithms() {
        ...
        parsesTo(
            "[ln ^ 2][x]",
            powerOf(naturalLogOf(xp("x"), powerInside = true), xp(2)),
        )
        parsesTo(
            "[ln[x] ^ 2]",
            powerOf(naturalLogOf(xp("x"), powerInside = false), xp(2)),
        )
        parsesTo(
            "[log_[a] ^ 2][x]",
            powerOf(logOf(xp("a"), xp("x"), powerInside = true), xp(2)),
        )
        parsesTo(
            "[log_[a][x] ^ 2]",
            powerOf(logOf(xp("a"), xp("x"), powerInside = false), xp(2)),
        )
        parsesTo(
            "[log ^ 2][x]",
            powerOf(logBase10Of(xp("x"), powerInside = true), xp(2)),
        )
        parsesTo(
            "[log[x] ^ 2]",
            powerOf(logBase10Of(xp("x"), powerInside = false), xp(2)),
        )
    }
