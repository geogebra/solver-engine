...
const isThroughStep = (trans) => trans.steps && trans.steps.length === 1 && trans.steps[0].path === trans.path;
...
const fetchDefaultTranslations = () => fetch("./DefaultTranslations.json")
    .then(resp => resp.json())
...
const renderPlanSelections = (selections, testSelections) => {
    ...
    <div class ="selections">${selections.length} plans found
        <ol>
            ${selections.map((selection) => `<li>${renderPlanSelection(selection, findTransformationInSelections(testSelections, selection.metadata.methodId))}</li>`).join('')}
        </ol>
    </div>
    `;
}
